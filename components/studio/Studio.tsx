...
const Studio = () => {
	...
	const AddNoteToTrack = (track: Track, note: Note) => {
		...
		let currentPartIndex = track.parts.findIndex((part) => part.startTime <= ((note.startColumn + 1) / noteLength) && part.stopTime >= ((note.startColumn + 1) / noteLength))
		...
		// If in not in any existing part, create a new part and add the note to it
			...
			const tonePart = new Tone.Part((time, value: any) => {
				track.sampler.triggerAttackRelease(value.note, value.duration, time, value.velocity);
			}, []).start(startTime);
	...
	const MoveNote = (partIndex: number, noteIndex: number, column: number, row: number) => {
		...
		if (part.startTime <= ((note.startColumn + 1) / noteLength) && part.stopTime >= ((note.startColumn + 1) / noteLength)) {
			note.startColumn -= part.startTime * noteLength;
		...
		tracks[selectedIndex].sampler.triggerAttackRelease(key, `${note.duration}n`);
	};
